...
사용자가 완료를 알리면, 먼저 Ubuntu가 이미 설치되어 있는지 확인:
```bash
wsl -l -v
```
- Ubuntu가 목록에 있고 STATE가 Running 또는 Stopped → **Step 2를 건너뛰고** Step 3으로 진행
- Ubuntu가 목록에 없거나 "배포 없음" → Step 2 진행

---

### Step 2: Ubuntu 설치 (👤 사용자 조작 필요 — 조건부)

**Step 1에서 Ubuntu가 자동 설치되지 않은 경우에만 실행합니다.**
